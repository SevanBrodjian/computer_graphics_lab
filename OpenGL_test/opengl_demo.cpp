...
{
    // CUSTOM CODE
    // Ensure client-side arrays, not VBO offsets
    glBindBuffer(GL_ARRAY_BUFFER, 0);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
    // Be defensive: re-enable the arrays you depend on
    glEnableClientState(GL_VERTEX_ARRAY);
    glEnableClientState(GL_NORMAL_ARRAY);
    // GLint ab=0, eab=0;
    // glGetIntegerv(GL_ARRAY_BUFFER_BINDING, &ab);
    // glGetIntegerv(GL_ELEMENT_ARRAY_BUFFER_BINDING, &eab);
    // std::cerr << "AB=" << ab << " EAB=" << eab << "\n";
    // GLboolean v = glIsEnabled(GL_VERTEX_ARRAY);
    // GLboolean n = glIsEnabled(GL_NORMAL_ARRAY);
    // std::cerr << "VERTEX_ARRAY=" << v << " NORMAL_ARRAY=" << n << "\n";
...
int main(int argc, char* argv[])
{
    ...
    glewExperimental = GL_TRUE;                    // important on some drivers
    GLenum err = glewInit();
    if (err != GLEW_OK) {                          // hard fail if missing
        fprintf(stderr, "GLEW init error: %s\n", glewGetErrorString(err));
        return 1;
    }
    fprintf(stderr, "GL %s, GLEW ok\n", glGetString(GL_VERSION));
